...
def main():
    ...
    try:
        ...
        st.error(f"Fallo en la conexión: {e}"); return

    menu = st.sidebar.radio("Navegación", ["Mi Reporte Individual", "Dashboard CEO"])
    ...
    if menu == "Mi Reporte Individual":
        ...
        if st.text_input("Contraseña Admin", type="password") == PASSWORD_CEO:
            lideres = data['Nombre de la persona Evaluada'].unique()
            sel = st.selectbox("Seleccionar Líder:", lideres)
            if sel:
