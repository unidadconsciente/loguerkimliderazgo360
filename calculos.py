...
def process_hogan_logic(df, nombre_evaluado, mapeo, min_obs):
    # 1. Filtramos todo donde la persona sea la evaluada
    df_persona = df[df['Nombre de la persona Evaluada'] == nombre_evaluado]
    
    # 2. Identificamos la columna de relación (índice 6)
    col_relacion = df.columns[6] # "Tu relación con el evaluado"
    
    # 3. Separación: Autoevaluación vs El resto (Evaluación de los demás)
    ...
    for dominio, items in mapeo.items():
        ...
        for n in items:
            ...
            if col_pregunta:
                # Cálculo Evaluación de los demás (Barra Naranja)
                if df_otros[col_pregunta].count() >= min_obs:
                    val_otros = pd.to_numeric(df_otros[col_pregunta], errors='coerce').mean()
                    if pd.notnull(val_otros):
                        scores_otros.append(val_otros)
                        items_con_quorum += 1
                
                # Cálculo Autoevaluación (Barra Azul)
                if not df_auto.empty:
                    val_auto = pd.to_numeric(df_auto[col_pregunta].iloc[0], errors='coerce')
                    if pd.notnull(val_auto):
                        scores_auto.append(val_auto)

        cobertura = items_con_quorum / len(items)
        p_otros = sum(scores_otros) / len(scores_otros) if scores_otros else None
        p_auto = sum(scores_auto) / len(scores_auto) if scores_auto else None
        
        calidad = "Sólido" if cobertura >= 0.8 else "Cautela" if cobertura >= 0.5 else "Insuficiente"
        
        resultados.append({
            "Categoría": dominio,
            "Autoevaluación": round(p_auto, 2) if p_auto else None,
            "Evaluación de los demás": round(p_otros, 2) if p_otros else None,
            "Brecha (Gap)": round(p_auto - p_otros, 2) if (p_auto and p_otros) else None,
            "Cobertura": cobertura,
